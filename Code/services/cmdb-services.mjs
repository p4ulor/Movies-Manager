...
import * as dataMem from '../data/cmdb-data-elastic.mjs'
...
export async function createGroup(name, description, isPrivate, token){
    try {
        ...
        const userID = await getUserElasticIDByToken(token)
...
export async function getGroupList(skip, limit, token){
    try {
        const user = (await dataMem.tryFindUserBy_(false, token)).elasticID
        if(!skip)  skip = 0
        if(!limit) limit = 10 //todo use mem to do paging?
        //return user.groups.slice(skip, skip+limit).map(g => { return {id: g.id, name: g.name}})
        return dataMem.getGroupListOfAUser(skip, limit, user)
    } catch(e) { throw e }
}
...
export async function getTopMovies(numOfTopMovies, token){
    try {
        const userAPIKey = (await dataMem.tryFindUserBy_(false, token)).user.api_key
...
async function getUserElasticIDByToken(token){
    return  (await dataMem.tryFindUserBy_(null, token, null, )).elasticID
